...
def register():
    body = request.json 
    if body["verifyPassword"] != body["password"] : raise ValidationError("passwords do not match")
    del body["verifyPassword"]
    results = UserSchema().load(body)
    user_exists = User.query.filter(User.username == results["username"]).first() is not None 
    if user_exists: raise ValidationError("username already taken")
...
def get_new_token():
    ...
    if not old_token: raise ValidationError("token is required")
