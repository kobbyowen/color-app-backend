...
def _add_tags(color_id: int, tag_ids: list):
    if not tag_ids:
        return
    color_tags = [ColorTags(color_id=color_id, tag_id=tag_id)
                  for tag_id in tag_ids]
    for color_tag in color_tags:
        db_session.add(color_tag)
    db_session.commit()
...
def _check_color_exists(name, id=-1):
    ...
    if color_exists:
        raise ColorAppException(
            DUPLICATE_RESOURCE, "a color with same name exists")
...
def edit_color_tags(color_id):
    ...
    existing_tags = [Tag.query.get(color_tag.tag_id)
                     for color_tag in Color.query.get(color_id).tags]
