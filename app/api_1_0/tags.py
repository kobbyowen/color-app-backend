...
def remove_tag(tag_id):
    tag = Tag.query.get(tag_id).delete() 
    db_session.commit() 
    return Rest.success()

@api.route("/tags", methods=["DELETE"])
def remove_tags():
    tag_ids = request.json.get("tags_ids")
    if not tags_id: raise ValidationError("tag ids required")
    for tag_id in tag_ids :
        owns_resource(Tag, "tag_id")(lambda tag_id: None)(tag_id=tag_id)
    map(lambda tag: tag.delete() , map(lambda id : Tag.query.get(id), tag_ids))
    db_session.commit() 
    return Rest.success()
